...
conceptMap = [
    ...
    {
        "pattern": r"Bilateral|Unilateral",
...
for c in cards:
    ...
    for rule in conceptMap:
        if re.search(rule['pattern'], old_tip, re.IGNORECASE) or re.search(rule['pattern'], question, re.IGNORECASE):
            c['tip'] = random.choice(intros) + rule['replace']
            replaced = True
            break
            
    if not replaced:
        c['tip'] = random.choice(intros) + soften_text(old_tip)
...
# Replace Array
json_str = json.dumps(cards, ensure_ascii=False, indent=4)
inject_str = f"const ALL_CARDS = {json_str};\n"

new_html = re.sub(r'const ALL_CARDS = \s*\[[\s\S]*?\];', inject_str, html)

with open('flashcards.html', 'w', encoding='utf-8') as f:
    f.write(new_html)

print("Python fully formatted and injected all 123 conversational tips successfully.")
